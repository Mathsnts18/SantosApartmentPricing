...
class DataTransformation:
    ...
    def get_data_transformer_object(self, df):
        ...
        try:
            ...
            # Coluna nrCondominio
            df.loc[df['nrCondominio'] > 10000, 'nrCondominio'] = (
                df.loc[df['nrCondominio'] > 10000, 'nrCondominio'] / 1000
            )
            ...
            df = pd.concat(
                [df.iloc[:, :14], df['nrSuites'], dummies_amenities], axis=1
            )
            ...
            df['nrComodos'] = (
                df['nrBanheiros'] + df['nrQuartos'] + df['nrSuites']
            )
    ...
    def initiate_data_transformation(self, raw_path):
        try:
            ...
            train_df, test_df = train_test_split(
                df_transform, test_size=0.3, random_state=42
            )
    ...
    def feature_selection(self):
        try:
            ...
            rfe = RFECV(
                regressor, step=1, cv=5, scoring='neg_root_mean_squared_error'
            )
            ...
            processed_train_df.to_csv(
                self.data_transformation_config.processed_train_path,
                index=False,
                header=True,
            )
            processed_test_df.to_csv(
                self.data_transformation_config.processed_test_path,
                index=False,
                header=True,
            )
