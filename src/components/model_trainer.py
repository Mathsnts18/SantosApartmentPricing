...
class NewFeats(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        X['nrBanheiros_Suites'] = np.where(
            X['nrSuites'] >= 1,
            X['nrBanheiros'] / X['nrSuites'],
            X['nrBanheiros'],
        )
        X['nrVagas_Quartos'] = np.where(
            X['nrQuartos'] >= 1, X['nrVagas'] / X['nrQuartos'], X['nrVagas']
        )
        X['nrCondominio_Andar'] = np.where(
            X['nrAndar'] >= 1,
...
class ModelTrainer:
    ...
    def initiate_model_trainer(self, train_data, test_data):
        ...
        try:
            ...
            save_object(
                file_path=self.model_trainer_config.scaler_path, obj=scaler
            )

            X_train_transformed = pd.DataFrame(
                X_train_transformed, columns=rfe.get_feature_names_out()
            )
            X_test_transformed = pd.DataFrame(
                X_test_transformed, columns=rfe.get_feature_names_out()
            )
            ...
            with mlflow.start_run():
                ...
                mlflow.sklearn.log_model(
                    model,
                    'CatBoostRegressor',
                    input_example=X_train_transformed.iloc[[0]],
                )
    ...
    def save_model(self):
        try:
            ...
            version = max([
                int(i.version)
                for i in client.get_latest_versions('Price_Model')
            ])

            if not version:
                raise CustomException(
                    'Nenhuma versão de modelo salva, escolha um no MLflow'
                )
            ...
            save_object(
                file_path=self.model_trainer_config.model_file_path, obj=model
            )
