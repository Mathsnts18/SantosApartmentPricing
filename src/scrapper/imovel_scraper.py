...
class ImovelScraper:
    ...
    def filter_links(self):
        ...
        try:
            self.filtered_links = [
                ...
                if '/lancamento/' not in link
            ]

            os.makedirs(
                name=os.path.dirname(
                    p=self.imoveis_config.filtered_links_path
                ),
                exist_ok=True,
            )
            with open(
                'artifacts/filtered_links.csv', 'a', newline=''
            ) as arquivo:
                writer = csv.writer(arquivo)
                writer.writerows([link] for link in self.filtered_links)
            logging.info('Links filtrados com sucesso.')
            print(
                f'Links filtrados com sucesso! Total de links: {len(self.filtered_links)}'
            )
        except Exception as e:
            raise CustomException(error_message=e, error_detail=sys)
    ...
    def load_imoveis(self):
        ...
        try:
            os.makedirs(
                name=os.path.dirname(p=self.imoveis_config.imoveis_path),
                exist_ok=True,
            )
            if os.path.exists('data/raw/imoveis.json'):
                ...
                logging.info('Imóveis carregados com sucesso.')
                print(
                    f'Imóveis carregados com sucesso! Total de imóveis: {len(self.imoveis)}'
                )
            else:
    ...
    def scraper(self):
        ...
        try:
            ...
            for link in pbar:
                ...
                advertiser_code = driver.find_element(
                    By.CSS_SELECTOR, value='[data-cy="ldp-propertyCodes-txt"]'
                ).text
                advertiser_code = (
                    re.search(r':\s(.+)\s\|', advertiser_code)
                    .group()[1:-1]
                    .strip()
                )
                ...
                if advertiser_code and advertiser_code not in self.imoveis:
                    ...
                    created_at = driver.find_element(
                        By.CSS_SELECTOR,
                        value='[data-testid="listing-created-date"]',
                    ).text
                    created_at = re.search(
                        r'criado em (\d{1,2}) de (\w+) de (\d{4})', created_at
                    )
                    dia, mes_str, ano = created_at.groups()
                    mes_num = meses[mes_str.lower()]
                    imovel_info['dtCriacao'] = (
                        f'{int(dia):02d}/{mes_num}/{ano}'
                    )

                    # Código do anunciante
                    advertiser_code = driver.find_element(
                        By.CSS_SELECTOR,
                        value='[data-cy="ldp-propertyCodes-txt"]',
                    ).text
                    ...
                    imovel_info['imobiliaria'] = driver.find_elements(
                        By.CSS_SELECTOR,
                        value='[data-testid="official-store-redirect-link"]',
                    )[1].text

                    # Preço do imóvel
                    imovel_info['nrPreco'] = driver.find_element(
                        By.CSS_SELECTOR,
                        value='[data-testid="price-info-value"]',
                    ).text
                    ...
                    imovel_info['endereco'] = driver.find_element(
                        By.CSS_SELECTOR,
                        value='[data-testid="address-info-value"]',
                    ).text
            ...
            print(
                f'Imoveis carregados com sucesso! Total de imoveis: {len(self.imoveis)}'
            )
